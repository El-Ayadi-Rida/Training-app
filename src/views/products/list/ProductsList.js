...
import { Row, Col, Button, Dropdown, Form, Card, Badge, Pagination, Tooltip, OverlayTrigger, Spinner } from 'react-bootstrap';
...
const ProductsList = () => {
  ...
  const {programs , status , error} = useSelector((state) => state.program);
  ...
  return (
    <>
      ...
      <div className="page-title-container">
        <Row className="g-0">
          ...
          <Col className="col-auto mb-3 mb-sm-0 me-auto">
            <NavLink className="muted-link pb-1 d-inline-block hidden breadcrumb-back" to="/app">
      ...
      {/* List Items Start  */}
      {status === 'LOADING' &&  <div className="d-flex justify-content-center mb-5"> <Spinner animation="border" variant="success" /> </div>}
      {
        status ==='SUCCEED' && programs.map((program , index)=>{
          return(
            <Card className={`mb-2 ${selectedItems.includes(1) && 'selected'}`} key={program.id}>
